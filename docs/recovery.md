...
3. Named mise restore tasks, in configuration order.
4. `chezmoi diff` and `chezmoi apply`.
5. Explicit Kopia snapshot restores, when enabled.
...
dependencies. A profile can add ordered mise tasks for exceptional setup that
must occur after runtimes are installed.
